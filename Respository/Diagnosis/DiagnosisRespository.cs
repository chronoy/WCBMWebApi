                    ...
                    switch(diagnosisType)
                    {
                        ...
                        case "FM":
                            using (DanielFMDiagnosticDataDetail detail = _context.DanielFMDiagnosticDataDetails.FirstOrDefault(obj => obj.ID == loopID))
                            {
                                ...
                                details.Add(new DiagnosticDataDetail() { Name = "压力(>1000KPa)", Result = _context.DiagnosticFMResultDescriptions.First(obj => obj.ID == detail.P3).DescriptionCN, Value = NotAvailable.Contains(detail.P3) ? "N/A" : (detail.v3 * 1000).ToString("F4") });
